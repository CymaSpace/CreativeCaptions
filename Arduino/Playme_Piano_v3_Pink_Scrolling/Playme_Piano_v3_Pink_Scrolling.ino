...
void loop()
{
  ...
  if (myFFT.available()) {
    ...
    */
      matrix.fillScreen(0);
      matrix.setCursor(cursor_x, 4); // comment out & turn off text - setCursor(2,4) places text 2 columns from left and 4 rows from bottom
      ...
      matrix.print("PLAY ME!"); // comment out & turn off text
...
//      matrix.print("ME"); // comment out & turn off text 
      if(--cursor_x < -45)
        {
          ...
          if(++pass >= 8) pass = 0;
          matrix.setTextColor(drawRGB24toRGB565((fade * 255), (fade * 255), (fade * 255))); // comment out & turn off text
       // matrix.setTextColor(colors[pass]); // comment out & turn off text
      ...
      if (fade <= 0.7){
        if (fade_direction == false && fading == true){
          fadeholdtime = millis();
          fading = false;
        }else if (millis()- fadeholdtime >= 6000){
