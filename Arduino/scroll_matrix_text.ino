...
int x    = COLUMNS;  //holds the width of the matrix for counting colums
int pass = 0;  //used to calculate the number of characters read from the serial input

void loop ()
{
  matrix.fillScreen(0);  //clear the screen
  matrix.setCursor(x, 4);  //set the begining cursor position
  serialtochar();
  chartoscreen();
...
void chartoscreen()
{
  if (readString.length() > 0)  //if the number of bytes in readString are greater than 0
  {
    ...
    delay(100);  //wait a bit - sets scrolling speed
